...
import type { PhotographyEventPackage, EntertainerPricing, BanjantriluPricing } from "./vendor-category-config";
...
export function buildLiveVendorMap(
  ...
  for (const l of listings) {
    ...
    vendorMap[l.id as string] = {
      ...
      banjantriluPricing: (() => {
        const bp = l.banjantrilu_pricing as import('./vendor-category-config').BanjantriluPricing | null
        return bp && Array.isArray(bp.cards) && bp.cards.length > 0 ? bp : undefined
      })(),
      mehendiPricing: (() => {
...
export const useStore = create<AppState & LiveModeState & {
  ...
  selectMehendiOptions: (ritualId, categoryId, selection) => {
    const { _liveMode } = get()
    set((s) => ({
      ritualBoards: s.ritualBoards.map((b) =>
        b.id === ritualId
          ? { ...b, categories: b.categories.map((c) => c.id === categoryId ? { ...c, mehendiSelection: selection } : c) }
          : b
      ),
    }))
    if (_liveMode) {
