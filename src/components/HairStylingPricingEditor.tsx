import { type HairStylingPricing } from '@/lib/vendor-category-config'

interface Props {
  value: HairStylingPricing
  onChange: (v: HairStylingPricing) => void
}
...
/**
 * Vendor-facing editor for Hair Styling pricing — used in onboarding and the
 * edit-pricing screen. Bridal + groom are priced per look; guest per guest.
 */
export default function HairStylingPricingEditor({ value, onChange }: Props) {
  return (
    <div className="space-y-5">
      <PriceField
        label="Bridal hairstyling" hint="Price per look. Leave blank if not offered." unit="/ look" step={500}
        val={value.bridalPricePerLook} onChange={(n) => onChange({ ...value, bridalPricePerLook: n })}
      />
      <PriceField
        label="Groom hairstyling" hint="Price per look. Leave blank if not offered." unit="/ look" step={500}
        val={value.groomPricePerLook} onChange={(n) => onChange({ ...value, groomPricePerLook: n })}
      />
      <PriceField
        label="Guest hairstyling" hint="Price per guest. Leave blank if not offered." unit="/ guest" step={100}
