-- ════════════════════════════════════════════
-- Admin catalog share links (revocable, optional expiry)
-- ════════════════════════════════════════════
-- An admin generates a tokenised link to share a full-details vendor catalog for
-- a category with a client (offline events). The link is revocable anytime and
-- can auto-expire, so it can't quietly undermine the paywall if it circulates.
--   /catalog/<token>  →  resolves to a category via resolve_catalog_share()
...
-- Admins create / list / revoke with their normal authenticated session.
...
-- table / harvest tokens. Returns the category only while the link is valid
-- (not revoked, not expired); otherwise no rows.
